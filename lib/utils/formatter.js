...
var formatters = {
  ...
  date: function (value, options) {
    ...
    if (options.subtract) {
      date.subtract(1, options.subtract);
    }
    var val;
    if (options.calendar) {
      val = date.calendar();
  ...
  number: function (value, options) {
    ...
    if (!isNaN(Number(value))) {
      ...
      if (typeof options.sigfigs === 'number' && typeof options.dps === 'undefined') {
        options.dps = Math.min(Math.max(Math.ceil(options.sigfigs - 1 - utils.log10(value)), 0),
          options.sigfigs - 1);
      }
...
var format = function (value, formatter) {
  ...
  if (typeof formatters[formatter.type] === 'function' && value !== null && value !== undefined) {
    ...
    if (formatter.abbr && formatter.magnitude) {
      var unmagnituded = formatters[formatter.type](value, _.extend({}, formatter,
        {magnitude: false, pad: false}));
      if (val !== unmagnituded) {
...
var dateRangeFormatter = function (value, options) {
  ...
  var endOptions = _.defaults(_.clone(options), {
    subtract: 'day'
  });
  var end = formatters.date(value[1], endOptions);
