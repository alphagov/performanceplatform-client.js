...
function formatKeys (module) {
  ...
  for (var x = 0; x < dataList.length; x++) {
    ...
    if (_.isArray(module.axes.x.key)) {
      dataItem.formatted_date_range = formatter.format(getRangeFromKeys(dataItem, module.axes.x.key), 'date');
    } else {
...
function tabularData (module) {
  ...
  _.each(axes.y, function (yAxis) {
    ...
    yCol = applyKeyAndFormatToData(data, yAxis.key, module.format || module['format-options'], yCol);
