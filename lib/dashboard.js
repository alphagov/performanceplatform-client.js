    ...
    .then(_.bind(function (dashboard) {
      ...
      _.each(dashboard.modules, function (module) {
        if (module['module-type'] === 'kpi') {
          kpiList.push(this.getModule(module));
        }
      }, this);
...
function formatKeys (module) {
  ...
  for (var x = 0; x < dataList.length; x++) {
    ...
    var isNumberOrCurrency = module.format.type === 'number' || module.format.type === 'currency';

    // formatted_change_from_previous
    if (previousValue && isNumberOrCurrency) {
      dataItem.formatted_change_from_previous = formatter.format(
