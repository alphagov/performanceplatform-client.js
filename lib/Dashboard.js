...
Dashboard.getAllSlugs = function () {
  var options = {
    json: true,
    url: config.stagecraft + 'public/dashboards'
  };

  return rp(options).then(function (dashboardList) {
    return _.pluck(dashboardList.items, 'slug');
  });
};

Dashboard.getSlugsByDepartment = function (department) {
  var options = {
    json: true,
    url: config.stagecraft + 'public/dashboards'
  };
  ...
  return rp(options).then(function (dashboardList) {
    return _.pluck(_.where(dashboardList.items, {
      department: {
        abbr: department
      }
    }), 'slug');
...
Dashboard.getModuleConfigsFromDashboards = function (listOfSlugs, moduleType) {
  var resolvedDashboards = [];

  _.each(listOfSlugs, function (slug) {
    var dashboard = new Dashboard(slug);
    resolvedDashboards.push(dashboard.getConfig());
  });

  return Q.allSettled(resolvedDashboards).then(function (results) {
    var dashboardsWithMatches = [];
    _.each(results, function (result) {
      if (result.state === 'fulfilled') {
        ...
        _.each(dashboard.modules, function (module) {
          if (module['module-type'] === moduleType) {
            matchedModules.push(module);
          }
        });

        if (matchedModules.length) {
          dashboard.modules = matchedModules;
          dashboardsWithMatches.push(dashboard);
        }
      }
    });

    return dashboardsWithMatches;
  });
};
